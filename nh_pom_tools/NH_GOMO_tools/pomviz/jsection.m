...
if strcmp(vname,'u')
  if jindex == ny
    var=mexnc('varget',ncid,vname,[(itime-1) 0 (jindex-1) 0],...
             [1 nz-1 1 nx],1);
  else
%
% Average to cell centers:
%
    var(:,:,1)=mexnc('varget',ncid,vname,[(itime-1) 0 (jindex-1) 0],...
                    [1 nz-1 1 nx],1);
    var(:,:,2)=mexnc('varget',ncid,vname,[(itime-1) 0 jindex 0],...
                    [1 nz-1 1 nx],1);
    var=sum(var,3)/2;
  end
else
...
%
h=mexnc('varget',ncid,'h',[(jindex-1) 0],[1 nx],1);
%
elb=mexnc('varget',ncid,'elb',[(itime-1) (jindex-1) 0],[1 1 nx],1);
%
fsm=mexnc('varget',ncid,'fsm',[(jindex-1) 0],[1 nx],1);
%
ind=find(fsm==0);
h(ind)=h(ind)*0;
%
...
% Get grid interval:
%
dx=mexnc('varget',ncid,'dx',[(jindex-1) 0],[1 nx],1);
%
% Integrate dx:
%
xgrid=[0 ; cumsum(dx,1)];
