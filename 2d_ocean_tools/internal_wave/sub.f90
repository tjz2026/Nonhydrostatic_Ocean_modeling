...
!N2 = 0.5e-3 ! stability frequency squared 
N2 = 3.5e-3 ! stability frequency squared 
...
DO k = 21,40
 depth(k) = depth(k)-40.0*(1.-COS(REAL(k-20)/20.*2.*PI))
END DO
...
REAL :: div, div1, div2
REAL :: dif1, dif2, difh, difv, diff,difz
REAL :: force
REAL,intent(in) :: time
...
force = dt*G*0.15/REAL(nx)/dx
force = force*sin(2*3.14159*time/1600.)
write(*,*) " force = ",force
...
DO k = 1,nx
  IF(wet(i,k))THEN
    ...
    IF(wet(i,k+1)) ustar(i,k) = u(i,k) + dt*pressx + advx(i,k)+force*exp(-2.0*(1.0*i/nz))
